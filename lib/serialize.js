...
const { NAMESPACES, VOID_ELEMENTS, NODE_TYPES, INVALID_XML_CHAR } = require("./constants");
...
function recordNamespaceInformation(element, map, prefixMap) {
  ...
  for (let i = 0; i < element.attributes.length; ++i) {
    ...
    if (attr.namespaceURI === NAMESPACES.XMLNS) {
      ...
      if (
        map.has(namespaceDefinition) &&
        map.get(namespaceDefinition).includes(attr.localName)
      ) {
        continue;
      }
      if (!map.has(namespaceDefinition)) {
        map.set(namespaceDefinition, []);
      }
      map.get(namespaceDefinition).push(attr.localName);
      prefixMap.set(attr.localName, attr.value);
    }
  }
...
function serializeDocumentType(node, namespace, prefixMap, requireWellFormed) {
  ...
  if (
    requireWellFormed &&
    (INVALID_XML_CHAR.test(node.systemId) ||
...
function serializeProcessingInstruction(
  ...
  if (
    requireWellFormed &&
    (INVALID_XML_CHAR.test(node.data) || node.data.includes("?>"))
  ) {
...
function serializeText(node, namespace, prefixMap, requireWellFormed) {
  if (requireWellFormed && INVALID_XML_CHAR.test(node.data)) {
...
function serializeElement(node, namespace, prefixMap, requireWellFormed, refs) {
  ...
  const map = new Map(Array.from(prefixMap, ([ns, prefixes]) => [ns, [...prefixes]]));
  const localPrefixesMap = new Map();
...
module.exports = (root, { requireWellFormed = false } = {}) => {
  const namespacePrefixMap = new Map([[NAMESPACES.XML, ["xml"]]]);
